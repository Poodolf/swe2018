 ...
 */
@LoadMe
public final class Monster extends Enemy {			
	...
	private static Vec JUMP_VELOCITY = new Vec(8.0f, 0.0f);
	...
	/**
	 * Prototype constructor used to dynamically {@link Monster#create(Vec, String...)}
	 ...
	 */
	public Monster(Vec startPos) {
		super(startPos, new Vec(), Monster.SIZE_OUTER);

		this.radius = new float[Monster.NUM_SALAMI];
		for (int i = 0; i < Monster.NUM_SALAMI; i++) {
			this.radius[i] = 0.1f + Monster.RNG.nextFloat() * 0.15f;
		}
	}

	@Override
	public void internalRender(GameGrid f) {
		f.drawCircle(this.getPos(), this.getSize(), Monster.COLOR_OUTER);

		float phi = this.currentAngle;
		for (int i = 0; i < Monster.NUM_SALAMI; i++) {
			// get polar coordinates
			phi += 2 * Math.PI / Monster.NUM_SALAMI;
	...
	protected void innerLogicLoop() {
		...
		this.currentAngle += this.deltaTime * Monster.ANGLE_SPEED;
	}
	...
	public void reactToCollision(GameElement element, Direction dir) {
		...
		if (dir == Direction.UP) {
			...
			this.getScene().getPlayer().addPoints(Monster.POINTS);
			// Let the player jump 
			element.killBoost();
			// kill the enemy
			this.addDamage(1);
	...
	public void collidedWithSolid(Frame collision, Direction dir) {
		...
		// upward push
		this.setVel(Monster.JUMP_VELOCITY);
	}

	@Override
	public Monster create(Vec startPos, String... options) {
		return new Monster(startPos);
	}
}